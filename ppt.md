...
프롬프트, 이메일, 요청서 등 비정형 텍스트를 입력받아 **python-pptx**로 완성도 높은 `.pptx` 기획안을 생성하는 스킬. NC 브랜드 디자인 시스템(Pretendard · NC 키컬러 · 플랫/미니멀)을 따른다.
...
- 나쁨: "NC톤은 Claude API를 활용해 NC 프로덕트에 접목할 수 있는 서비스 프로토타입을 만드는 사내 해커톤입니다."
- 좋음: "Claude API로 NC 프로덕트에 접목할 서비스 프로토타입을 만드는 사내 해커톤"
...
- 불릿은 단순 나열이 아니라 `리드: 설명` 단위로 작성한다. 예: `공급 제약: 채용만으로 해소되지 않는 FDE 공급 부족`.
...
타임라인·stats·table·two-col 등 각 레이아웃의 전체 구현은 위 헬퍼로 조립한다 (대표 구현은 본 스킬이 생성한 NC톤 기획안 스크립트 참조).
...
📊 슬라이드: 14장
📋 유형: 범용 기획안 (general)
🎨 테마: NC 스타일 (Pretendard · Navy/Sky · 플랫 · 명사형 종결)
🔍 검증: 14장 전체 렌더링 확인 완료
